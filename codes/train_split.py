...
def main():
    ...
    for epoch in range(start_epoch, total_epochs + 1):
        ...
        for _, train_data in enumerate(train_loader):
            ...
            #### training
            model.feed_data(train_data, split=True)
            model.optimize_parameters(current_step, split=True)
            ...
            if opt['datasets'].get('val', None) and current_step % opt['train']['val_freq'] == 0:
                if opt['model'] in ['sr', 'srgan'] and rank <= 0:  # image restoration validation
                    ...
                    if opt['dist']:
                        ...
                        for val_data in val_loader:
                            model.feed_data(val_data, split=True)
                            model.test(split=True)
                            visuals = model.get_current_visuals(split=True)
                            rlt_img_Y = util.tensor2img(visuals['rlt_Y'], reverse_channel=False)  # uint8
                            gt_img_Y = util.tensor2img(visuals['GT_Y'], reverse_channel=False)  # uint8
                            rlt_img_UV = util.tensor2img(visuals['rlt_UV'], reverse_channel=False)  # uint8
                            gt_img_UV = util.tensor2img(visuals['GT_UV'], reverse_channel=False)  # uint8
                            # calculate PSNR
                            psnr_y_sum += util.calculate_psnr(rlt_img_Y, gt_img_Y)
                            psnr_u_sum += util.calculate_psnr(rlt_img_UV[:, :, 0], gt_img_UV[:, :, 0])
                            psnr_v_sum += util.calculate_psnr(rlt_img_UV[:, :, 1], gt_img_UV[:, :, 1])
                            pbar.update()
